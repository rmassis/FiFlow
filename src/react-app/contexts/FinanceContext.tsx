    ...
    persist(
        (set, get) => ({
            ...
            updateBankAccount: async (id, updates) => {
                ...
                if (updatedAccount) {
                    try {
                        const { error } = await supabase.from('bank_accounts').update(mapBankAccountToDB(updatedAccount)).eq('id', id);
            ...
            updateCreditCard: async (id, updates) => {
                ...
                if (updatedCard) {
                    try {
                        const { error } = await supabase.from('credit_cards').update(mapCreditCardToDB(updatedCard)).eq('id', id);
                        if (error) console.error("DB Update Error", error);
                    } catch (err) {
                        console.error("DB Update Error", err);
                    }
